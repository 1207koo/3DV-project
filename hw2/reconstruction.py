...
def FindMissingReconstruction(X, track_i):
    ...
    """
    invalid1 = np.prod(X == -1, axis=-1)
    valid1 = (1 - invalid1).astype(bool)

    invalid2 = np.prod(track_i == -1, axis=-1)
    valid2 = (1 - invalid2).astype(bool)

    #new_point = valid1 * valid2
    new_point = invalid1.astype(bool) * valid2
    return new_point
